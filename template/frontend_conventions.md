...
- 반환 타입을 명시한다.
- `"use client"` 지시어는 **Next.js App Router(RSC) 프로젝트에서만** 파일 최상단에 선언한다. Vite 등 RSC를 쓰지 않는 프로젝트에서는 선언하지 않는다.
...
### 업데이트 대상
- `README.md`: 설치/실행 방법, 환경변수, 스크립트 명령이 바뀌면 반드시 갱신한다.
- 컴포넌트/훅 문서(Storybook, JSDoc 등): props·반환 타입·동작이 바뀌면 갱신한다.
- API 연동 문서: 엔드포인트, 요청/응답 형태가 바뀌면 프론트 사용처 문서도 함께 갱신한다.
- `.env.example`: 새 환경변수를 추가하면 즉시 반영한다. 실제 값은 넣지 않는다.

### 규칙
- 공개 컴포넌트/훅/유틸의 시그니처가 바뀌면, 이를 사용하는 예시 코드가 문서에 있는 경우 함께 수정한다.
- 새 기능(페이지, 주요 컴포넌트)을 추가하면 사용법을 1개 이상 예시와 함께 문서화한다.
- 삭제/이름 변경된 심볼은 문서에서도 제거·수정한다. 죽은 예시를 남기지 않는다.

```
// Bad - 코드만 바꾸고 README의 환경변수 설명은 그대로
// Good - VITE_API_URL 추가 시 README + .env.example 동시 갱신
```
...
### 도구
- 유닛/컴포넌트: Vitest + React Testing Library.
- E2E: Playwright.
...
### 작성 규칙
- **동작 중심**으로 테스트한다. 구현 세부(내부 state 이름 등)가 아니라 사용자가 보는 결과·상호작용을 검증한다.
- 쿼리는 접근성 우선(`getByRole`, `getByLabelText`)으로 찾는다. `getByTestId`는 대안이 없을 때만.
- API 호출은 MSW 등으로 목킹한다. 컴포넌트 테스트에서 실제 네트워크를 타지 않는다.
- 최소 커버리지: 정상 흐름 + 에러 상태 + 빈 상태(§11 패턴) 각 1개 이상.
- 버그 수정 시 **해당 버그를 재현하는 회귀 테스트**를 반드시 추가한다.

```tsx
// Good - 동작/접근성 중심
it("에러 시 재시도 버튼을 노출한다", async () => {
  render(<PostList />);
...
### 금지 사항
- 스냅샷 테스트로 검증을 대체하는 것(회귀 감지 보조 용도로만 허용).
- 테스트를 비활성화(`.skip`, `xit`)한 채 커밋하는 것. 부득이하면 사유를 주석으로 남긴다.
